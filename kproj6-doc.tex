...
\end{quote}
  ...
  Although TDS seems to support everything that is needed for this project, it's installation and especially configuration for agreggation and special needs is not trivial. Moreover there are doubts about speed and dealing with serving a range containing large quantitied of points. Advantage is, that SINTEF currently have a TDS running and configured, so we won't need to configure it from scratch and SINTEF employees probably have experience and knowledge about setting it up which we lack.
